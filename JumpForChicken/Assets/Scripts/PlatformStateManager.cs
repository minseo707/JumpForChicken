...
public class PlatformStateManager : MonoBehaviour
{
    ...
    void Awake()
    {
        blockStore = FindAnyObjectByType<BlockStore>();
        ...
        if (blockStore == null) Debug.LogError("[BlockStore] No BlockStore found");
            ...
            if (playerRigidbody != null && isPlayerOnPlatform)
            {
                ...
                if (timeOnPlatform >= requiredTimeOnPlatform && gameObject.layer != currentPlatformLayer && !stateChanged)
                {
                    if (IsPlayerLanding())
                    {
                        ...
                        if (stateChanged){
                            // 레이어 설정
                            blockStore.ChangeBlockStateLayer(blockNumber);
                            ResetStateChanged();
                        }
    ...
    public void SetOnlyPlatformLayer(int layer){
